...
class TextEmbedding(nn.Module):
    def __init__(self,model_url,use_SenticnetFeature):
        ...
        self.use_senticnet = use_SenticnetFeature
...
class Tower(nn.Module):
    def __init__(self, expert_dim, num_labels,drop):
        super(Tower, self).__init__()
        self.head = nn.Sequential(
            nn.Linear(expert_dim, 100),
            nn.LayerNorm(100),
            nn.GELU(),
            nn.Dropout(drop),
            nn.Linear(100, 50),
