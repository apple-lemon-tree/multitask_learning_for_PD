...
from SingleTaskDataLoad.load_singledata import *
...
parser.add_argument("--model_name",type=str,default="D:/models/electra-base-discriminator",help="分词和训练的模型")
...
parser.add_argument("--learning_rate1",type=float,default=1e-6,help="学习率1")
parser.add_argument("--learning_rate2",type=float,default=5e-5,help="学习率2")
parser.add_argument("--eps",type=float,default=1e-8,help="数值稳定参数")
parser.add_argument("--patience",type=int,default=5,help="早停忍耐度")
parser.add_argument("--personality_url",type=str,default="D:/datasets_1/Essays/原始数据/essays_1.csv",help="人格数据集路径")
...
for fold, (p_train_index, p_test_index) in enumerate(pnd_K):
    ...
    # 加载嵌入模型
    model = SingletaskModel(args.model_name).to(device)
    # 计算参数
    ...
    optimizer = optim.AdamW([
        {'params': model.electra.parameters(), 'lr': args.learning_rate1},
    ...
    for epoch in range(args.epoch):
        ...
        for batch in tqdm(pnd_train_dataloader, desc="Train..."):
            ...
            sentic_fea = batch['sentic_fea'].to(device)
