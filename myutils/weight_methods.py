...
class NashMTL_CPU(WeightMethod):
    def __init__(
        self,
        n_tasks: int,
        device: torch.device,
        ...
        super(NashMTL_CPU, self).__init__(
            n_tasks=n_tasks,
            device=device,
        )
        ...
        **kwargs,
        ...
        if (self.step % self.update_weights_every) == 0:
            ...
            for i, loss in enumerate(losses):
                ...
                grad = torch.cat([torch.flatten(grad) for grad in g if grad is not None])
                grads[i] = grad
...
class NashMTL_GPU(WeightMethod):
    def __init__(
        self,
        n_tasks: int,
        device: torch.device,
        ...
        super(NashMTL_GPU,self).__init__(
            n_tasks=n_tasks,
            device=device,
        )
        ...
        # keep placeholders for legacy compatibility
        self.alpha_param = None
        self.prvs_alpha_param = None
        self.G_param = None
        self.normalization_factor_param = None

    # --------------------------
    # NEW: torch-based optimizer
    # --------------------------
    def solve_optimization_torch(self, gtg: torch.Tensor):
        """
        Solve for alpha using a GPU-friendly projected gradient descent with penalty.
        Inputs:
            gtg: (n_tasks, n_tasks) torch tensor on the same device as self.device.
        Returns:
            alpha: numpy array (n_tasks,)
        NOTE:
            This is an approximate solver that optimizes:
                minimize sum(G @ alpha) + penalty_coef * sum(relu(1/(alpha*norm*(G@alpha)) - 1)**2)
            with projection alpha >= eps_alpha.
        """
        device = gtg.device
        n = gtg.shape[0]

        # initialize alpha (positive)
        alpha = torch.tensor(self.prvs_alpha, dtype=torch.float32, device=device)
        alpha = alpha.clamp(min=self.eps_alpha).requires_grad_(True)

        # normalization factor: use previously computed normalization
        norm_factor = float(self.normalization_factor) if hasattr(self, "normalization_factor") else 1.0
        norm_factor = float(norm_factor) if isinstance(norm_factor, (float, int, np.floating, np.integer)) else float(norm_factor.item())

        # simple optimizer (SGD-like)
        optimizer = torch.optim.SGD([alpha], lr=self.torch_lr)

        for it in range(self.optim_niter):
            optimizer.zero_grad()

            # compute G alpha and objective
            G_alpha = gtg.matmul(alpha)  # shape (n,)
            obj_linear = torch.sum(G_alpha)  # sum over tasks

            # penalty for constraints: alpha * norm * G_alpha >= 1  ->  violation = relu(1/(alpha*norm*G_alpha) - 1)
            # to avoid division by zero, keep small eps
            safe_alpha = alpha.clamp_min(self.eps_alpha)
            safe_G_alpha = G_alpha.clamp_min(self.eps_alpha)
            product = safe_alpha * (norm_factor * safe_G_alpha)

            # violation measure (if product < 1 then positive violation)
            violation = torch.relu(1.0 / product - 1.0)  # shape (n,)
            penalty = torch.sum(violation * violation)  # L2 penalty of violations

            loss_obj = obj_linear + self.penalty_coef * penalty

            # backprop and step
            loss_obj.backward()
            optimizer.step()

            # projection: ensure non-negativity and minimal alpha
            with torch.no_grad():
                alpha.data.clamp_(min=self.eps_alpha)
                # optional normalization to keep alpha in a reasonable scale
                # here we renormalize to have sum(alpha) == n (original code used roughly sum ~ n)
                s = alpha.sum()
                if s.item() > 0:
                    alpha.data = alpha.data * (float(n) / s.item())

        final_alpha = alpha.detach().cpu().numpy()
        # store for next warm start
        self.prvs_alpha = final_alpha.copy()
        return final_alpha

    # --------------------------
    # existing methods (mostly same as your original)
    # --------------------------
    def _init_optim_problem(self):
        # keep this as a placeholder to preserve class API; not used by torch solver.
        self.alpha_param = None
        self.prvs_alpha_param = None
        self.G_param = None
        self.normalization_factor_param = None
    ...
    def get_weighted_loss(
        ...
        **kwargs,
        ...
        if (self.step % self.update_weights_every) == 0:
            ...
            grads = {}
            trainable_shared = [p for p in shared_parameters if p.requires_grad]
            # compute gradients for each task wrt shared trainable parameters ONLY
            for i, loss in enumerate(losses):
                g = list(
                    torch.autograd.grad(
                        loss,
                ...
                # flatten and cat only existing grads
                grad = torch.cat([torch.flatten(grad) for grad in g if grad is not None]) if any(grad is not None for grad in g) else torch.zeros(0, device=self.device)
                grads[i] = grad

            # if grads are empty (no trainable shared params), fallback
            if len(grads[0]) == 0:
                # fallback to uniform weights
                alpha = np.ones(self.n_tasks, dtype=np.float32)
                alpha = torch.from_numpy(alpha)
            else:
                # stack into matrix G (T, P_sub)
                G = torch.stack(tuple(v for v in grads.values())).to(self.device)  # shape (T, P_sub)
                GTG = torch.mm(G, G.t())  # (T, T)

                # normalization factor (as original) -> move to cpu scalar
                self.normalization_factor = (
                    torch.norm(GTG).detach().cpu().numpy().reshape((1,))
                )
                # normalize GTG same as original
                if self.normalization_factor.item() != 0:
                    GTG_norm = GTG / float(self.normalization_factor.item())
                else:
                    GTG_norm = GTG

                # call torch-based solver (on same device as GTG_norm)
                alpha = self.solve_optimization_torch(GTG_norm.to(self.device))
                alpha = torch.from_numpy(alpha).float()

        else:
            self.step += 1
            alpha = torch.from_numpy(self.prvs_alpha).float()

        # ensure alpha on correct device and dtype
        alpha = alpha.to(self.device)
        weighted_loss = sum([losses[i] * alpha[i] for i in range(len(alpha))])
        extra_outputs["weights"] = alpha.detach().cpu().numpy()
        return weighted_loss, extra_outputs
    ...
    def backward(
            ...
            task_specific_parameters: Union[
                List[Union[List[torch.nn.parameter.Parameter], torch.Tensor]], torch.Tensor
            ] = None,
            last_shared_parameters: Union[
                List[torch.nn.parameter.Parameter], torch.Tensor
            ] = None,
        ...
        # 2) 显式对 shared parameters 求梯度并赋值到 p.grad
        #    以避免调用 loss.backward() 造成对所有参数的隐式求导
        if shared_parameters is None:
            trainable_shared = []
        else:
            # ensure list and keep only requires_grad
            trainable_shared = [p for p in shared_parameters if p.requires_grad]

        if len(trainable_shared) > 0:
            ...
            # assign grads (detach to avoid creating extra graph)
            for p, g in zip(trainable_shared, shared_grads):
                if g is None:
                    p.grad = None
                else:
                    p.grad = g.detach().clone()

        # 3) 对每个任务的 task-specific 参数计算其单独 loss 的梯度并写入 .grad
        #    Expect task_specific_parameters to be a list (len == n_tasks), each element is an iterable of params
        if task_specific_parameters is not None:
            # convert to list for indexing if it's not already
            ts_params_list = list(task_specific_parameters)

            for i in range(len(ts_params_list)):
                params_i = ts_params_list[i]
                if params_i is None:
                    continue
                params_i = [p for p in params_i if p.requires_grad]
                if len(params_i) == 0:
                    continue
                ...
                for p, g in zip(params_i, grads_i):
                    if g is None:
                        continue
                    # accumulate if grad already exists (e.g., some param shared elsewhere)
                    if p.grad is None:
                        p.grad = g.detach().clone()
                    else:
                        p.grad = p.grad + g.detach().clone()

        # 4) 梯度裁剪（先对 shared，再对 task-specific）
        if self.max_norm > 0:
            if len(trainable_shared) > 0:
                torch.nn.utils.clip_grad_norm_(trainable_shared, self.max_norm)

            if task_specific_parameters is not None:
                all_task_params = []
                for params in task_specific_parameters:
                    if params is None:
                        continue
                    all_task_params.extend([p for p in params if p.requires_grad])
                if len(all_task_params) > 0:
                    torch.nn.utils.clip_grad_norm_(all_task_params, self.max_norm)
    ...
    def get_weighted_loss(
        ...
        **kwargs,
        ...
        for i, loss in enumerate(losses):
            ...
            """
            valid_grads = [torch.flatten(grad) for grad in g if grad is not None]
            grad = torch.cat(valid_grads)
            grads[i] = grad
            """
            grads[i] = [torch.flatten(grad) for grad in g if grad is not None]
